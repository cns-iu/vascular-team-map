...
psql --no-align --tuples-only --field-separator ' ' -c "SELECT author_id, replace(lower(first_name || ' ' || last_name), ' ', '-') AS name FROM authors_for_search" | while read author_id name
do
